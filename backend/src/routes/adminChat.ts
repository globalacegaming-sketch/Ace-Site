...
import { notifyUserSupportMessage } from '../services/chatNotificationService';
...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const notifyBody =
        sanitizedMessage || (attachment ? 'Support sent an attachment' : 'New support message');
      void notifyUserSupportMessage({ userId, body: notifyBody });
...
router.post('/:id/reactions', async (req: Request, res: Response) => {
  try {
    ...
    // Find if admin already reacted with this exact emoji (toggle off) or any other emoji (replace)
    ...
    if (sameEmojiIdx >= 0) {
      ...
      // Remove any previous reaction by this admin (one reaction per user)
      const prevIdx = message.reactions.findIndex(
        (r: any) => r.reactorId === reactorId && r.reactorType === 'admin'
      );
