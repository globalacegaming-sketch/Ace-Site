...
async function updateFortunePandaUsernames() {
  try {
    // Connect to MongoDB
    ...
    for (const user of users) {
      ...
      // Check if username already ends with _GAGame
      if (user.fortunePandaUsername.endsWith('_GAGame')) {
        skipped.push({ 
          userId: user._id, 
          username: user.username,
          fpUsername: user.fortunePandaUsername,
          reason: 'Already has _GAGame suffix' 
        });
        continue;
      }

      // Append _GAGame suffix
      const newUsername = `${user.fortunePandaUsername}_GAGame`;
